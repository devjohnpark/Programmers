// 핵심 아이디어
// 구하는 답: 당첨 제외될 제제 아이디의 조합 (user_id중에서 banned_id에 포함되는 경우의 수)
...
// depth == banned_id의 길이시 모두 선택 완료해서 하나의 조합 완성
// 선택된 후보(user_id)는 HashSet에 저장해서 이미저정되어 있다면 후보군 제외해서 중복을 피한다.
// 특정 banned_id와 매칭되는 user_id는 0 인덱스부터 확인해야하므로 순서 중복이 생긴다.
...
class Solution {
    ...
    private void backtracking(int depth, String[] user_id, String[] banned_id, Set<String> selectedUsers) {
        if (depth == banned_id.length) {
            result.add(new HashSet<>(selectedUsers)); // 복사본 저장
            return;
        }
